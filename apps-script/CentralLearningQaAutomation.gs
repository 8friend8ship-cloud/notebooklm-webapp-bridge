...
function inferExpectedOutputContractV1(fileMeta) {
  ...
  if (/\.(png|jpe?g|webp|gif)$/i.test(n) || /^image\//.test(mt)) {outputType='IMAGE';goal='usable visual asset with source traceability and template reuse value';templateKey='IMAGE_ASSET_T1';}
  else if (/\.(mp4|mov|webm|mkv)$/i.test(n) || /^video\//.test(mt) || /video/.test(n)) {outputType='VIDEO';goal='usable Flow/video result with source traceability and front-app reuse value';appId='APP_VTUBE_1011B';templateKey='VIDEO_PATTERN_T1';}
  else if (/\.(wav|mp3|m4a|aac|flac)$/i.test(n) || /^audio\//.test(mt) || /notebook.*audio|audio/.test(n)) {outputType='AUDIO';goal='usable NotebookLM/audio result with physical/Drive readback and learning value';appId='APP_NOTEBOOKLM_BRIDGE';templateKey='NLM_OUTPUT_TEMPLATE';}
  else if (/report|doc|text|json|analysis|audit|readback/.test(n) || /json|text|document|spreadsheet/.test(mt)) {outputType='TEXT_DATA';goal='complete analyzable result that can become Seed/Template evidence';templateKey='TEXT_ANALYSIS_V1';}
  else if (/flow/.test(n)) {outputType='MEDIA_DATA';goal='usable Flow result whose actual artifact type is verified before Seed/Template promotion';appId='APP_VTUBE_1011B';templateKey='FLOW_RESULT_GENERIC_V1';}
...
function runCentralDualAiReviewV1(ctx) {
  ...
  return {policy:policy,gemini:runGeminiLearningQaV1_(ctx,policy),openai:runOpenAiLearningQaV1_(ctx,policy)};
}
...
function centralAiUsagePolicyV1_() {
  ...
  return {
    ...
    maxChars:Math.max(1000,Math.min(16000,Number(p.getProperty('CENTRAL_AI_QA_MAX_CHARS')||8000)))
  };
}
...
function runGeminiLearningQaV1_(ctx, policy) {
  ...
  if (key && allowed.ok && /API/.test(policy.geminiMode)) return callGeminiQaApiV1_(ctx,key,policy,allowed);
...
function runOpenAiLearningQaV1_(ctx, policy) {
  ...
  if (key && allowed.ok && /API/.test(policy.openaiMode)) return callOpenAiQaApiV1_(ctx,key,policy,allowed);
...
function buildQaPromptV1_(ctx, maxChars) {
  const limit=Math.max(1000,Math.min(16000,Number(maxChars||8000)));
  const text=String(ctx.extracted.text||ctx.extracted.summary||'').slice(0,limit);
...
function callGeminiQaApiV1_(ctx,key,policy,slot) {
  ...
  const r=UrlFetchApp.fetch(url,{method:'post',contentType:'application/json',muteHttpExceptions:true,payload:JSON.stringify({contents:[{parts:[{text:buildQaPromptV1_(ctx,policy.maxChars)}]}],generationConfig:{temperature:0,responseMimeType:'application/json'}})});
...
function callOpenAiQaApiV1_(ctx,key,policy,slot) {
  ...
  const r=UrlFetchApp.fetch('https://api.openai.com/v1/responses',{method:'post',contentType:'application/json',headers:{Authorization:'Bearer '+key},muteHttpExceptions:true,payload:JSON.stringify({model:model,input:buildQaPromptV1_(ctx,policy.maxChars),max_output_tokens:1200})});
  const code=r.getResponseCode(), body=r.getContentText();
...
function queueUiAiReviewV1_(provider,ctx) {
  ...
  const policy=centralAiUsagePolicyV1_();
  const payload={provider:provider,task:'LEARNING_TEMPLATE_QA',fileId:ctx.fileMeta.fileId,fileUrl:ctx.fileMeta.url,prompt:buildQaPromptV1_(ctx,policy.maxChars),successCriteria:'JSON_QA+DRIVE_READBACK',usagePolicy:'SUBSCRIPTION_UI_FIRST;API_ONLY_IF_CAP_PROPERTY>0'};
...
function testCentralLearningQaFunctionsV1() {
  const flowImage={fileId:'FIXTURE_IMAGE',name:'flow_result_preview.png',mimeType:'image/png',size:1024,url:'https://drive.google.com/file/d/FIXTURE_IMAGE',updatedAt:new Date()};
  const flowVideo={fileId:'FIXTURE_VIDEO',name:'flow_result_video.mp4',mimeType:'video/mp4',size:2048,url:'https://drive.google.com/file/d/FIXTURE_VIDEO',updatedAt:new Date()};
  const textFixture={fileId:'FIXTURE_TEXT',name:'notebooklm_result_analysis.json',mimeType:'application/json',size:1024,url:'https://drive.google.com/file/d/FIXTURE_TEXT',updatedAt:new Date()};
  const ci=inferExpectedOutputContractV1(flowImage), cv=inferExpectedOutputContractV1(flowVideo), ct=inferExpectedOutputContractV1(textFixture);
  if(ci.outputType!=='IMAGE') throw new Error('FLOW_PNG_CLASSIFICATION_FAIL:'+ci.outputType);
  if(cv.outputType!=='VIDEO') throw new Error('FLOW_MP4_CLASSIFICATION_FAIL:'+cv.outputType);
  if(ct.outputType!=='TEXT_DATA') throw new Error('TEXT_CLASSIFICATION_FAIL:'+ct.outputType);
  const det=scoreCentralOutputDeterministicV1({name:textFixture.name,size:textFixture.size,mimeType:textFixture.mimeType,hasText:true,text:'result evidence seed template'},ct);
  ...
  if(policy.geminiDailyCap<0 || policy.openaiDailyCap<0) throw new Error('NEGATIVE_CAP_FAIL');
  return {ok:true,imageContract:ci,videoContract:cv,textContract:ct,deterministic:det,merged:merged,apiPolicy:policy,defaultApiBlocked:(policy.geminiDailyCap===0&&policy.openaiDailyCap===0)};
}
