...
$claspCmd=Get-Command clasp.cmd -ErrorAction SilentlyContinue; if(-not $claspCmd){$claspCmd=Get-Command clasp -ErrorAction SilentlyContinue}; if(-not $claspCmd){throw 'CLASP_COMMAND_NOT_FOUND_EXISTING_RUNNER_REQUIRED'}
& $claspCmd.Source show-authorized-user --json *> $null; if($LASTEXITCODE -ne 0){& $claspCmd.Source show-authorized-user *> $null; if($LASTEXITCODE -ne 0){throw 'EXISTING_CLASP_AUTH_NOT_AVAILABLE_DO_NOT_LOGIN'}}
$listText=(& $claspCmd.Source list-scripts 2>&1|Out-String); if($LASTEXITCODE -ne 0){throw 'CLASP_LIST_SCRIPTS_FAILED'}
$lines=@($listText -split "`r?`n"|Where-Object{$_ -match [regex]::Escape($TargetTitle)}); if($lines.Count -eq 0){throw "SCRIPT_TITLE_NOT_FOUND:$TargetTitle"}; if($lines.Count -gt 1){throw "SCRIPT_TITLE_AMBIGUOUS:$TargetTitle"}
$match=[regex]::Match($lines[0],'[A-Za-z0-9_-]{30,}'); if(-not $match.Success){throw 'SCRIPT_ID_PARSE_FAILED'}; $scriptId=$match.Value
$stamp=Get-Date -Format 'yyyyMMdd_HHmmss'; $snapshotDir=Join-Path $env:TEMP "central-bound-readonly-$TargetTitle-$stamp"; New-Item -ItemType Directory -Force -Path $snapshotDir|Out-Null
Push-Location $snapshotDir; try{& $claspCmd.Source clone-script $scriptId *> $null; if($LASTEXITCODE -ne 0){& $claspCmd.Source clone $scriptId *> $null; if($LASTEXITCODE -ne 0){throw 'CLASP_CLONE_EXISTING_SOURCE_FAILED'}}}finally{Pop-Location}
$deploymentText=(& $claspCmd.Source list-deployments $scriptId 2>&1|Out-String); if($LASTEXITCODE -ne 0){throw 'CLASP_LIST_DEPLOYMENTS_FAILED'}; if($deploymentText -notmatch [regex]::Escape($ExpectedDeploymentId)){throw 'EXPECTED_DEPLOYMENT_ID_NOT_FOUND_ABORT'}
$sourceRoot=$snapshotDir; $claspPath=Join-Path $snapshotDir '.clasp.json'; if(Test-Path $claspPath){$cfg=Get-Content -Raw -LiteralPath $claspPath|ConvertFrom-Json; if($cfg.rootDir){$sourceRoot=[IO.Path]::GetFullPath((Join-Path $snapshotDir ([string]$cfg.rootDir)))}}
