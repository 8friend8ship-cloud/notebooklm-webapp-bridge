...
async function wakeControlCenter(cfg,reason="AUTH_REQUIRED"){
  const now=Date.now();const a=(await chrome.storage.local.get(AUTH_KEY))[AUTH_KEY]||{};
  await chrome.storage.local.set({[AUTH_KEY]:{reason,updatedAt:new Date().toISOString(),lastWakeAt:a.lastWakeAt||0}});
  if(now-Number(a.lastWakeAt||0)<120000)return;
  await chrome.storage.local.set({[AUTH_KEY]:{reason,updatedAt:new Date().toISOString(),lastWakeAt:now}});
  const base=String(cfg?.frontendOrigin||CONTROL_ORIGIN).replace(/\/$/,"");
  try{const tabs=await chrome.tabs.query({url:`${base}/*`});if(tabs[0]?.id)await chrome.tabs.update(tabs[0].id,{url:`${base}/`,active:true});else await chrome.tabs.create({url:`${base}/`,active:true});}catch{}
}
...
async function recoverStale(cfg,token,tasks){const recovered=[];for(const t of tasks){if(!staleClaim(t))continue;const id=taskId(t);try{await api(cfg.appsScriptUrl,{action:"updateTask",sessionToken:token,taskId:id,status:"RETRY",patch:{error:`ASYNC_STALE_CLAIM_RECOVERED_AFTER_${timeoutSeconds(t)}s`,clearClaim:true,claimedAt:"",startedAt:""}});recovered.push({...t,status:"RETRY",STATUS:"RETRY",claimedAt:"",CLAIMED_AT:"",startedAt:"",STARTED_AT:""});}catch{}}return recovered;}
async function finalize(cfg,token,active){
  const id=String(active?.taskId||"");if(!id){await setActive(null);return {state:"EMPTY"};}
  let host;try{host=await hostResult(id);}catch(error){const age=Date.now()-Number(active.startedAtMs||Date.now());const hard=(Number(active.timeoutSeconds||DEFAULT_TIMEOUT)+120)*1000;if(age<=hard)return {state:"WAIT_HOST",error:String(error?.message||error)};try{await hostCancel(id);}catch{};try{await api(cfg.appsScriptUrl,{action:"updateTask",sessionToken:token,taskId:id,status:"ERROR",patch:{error:"LOCAL_ASYNC_HOST_RESULT_TIMEOUT"}});}catch{};await setActive(null);return {state:"TIMEOUT"};}
  if(["RUNNING","STARTED"].includes(String(host.state||"")))return {state:String(host.state),taskId:id};
  if(host.state==="DONE"){
    const result=host.result||{};
    try{
      if(result.ok)await api(cfg.appsScriptUrl,{action:"completeTask",sessionToken:token,taskId:id,result:{resultText:JSON.stringify(result,null,2),resultUrls:[],notebookUrl:""}});
      else await api(cfg.appsScriptUrl,{action:"updateTask",sessionToken:token,taskId:id,status:"ERROR",patch:{error:String(result.stderr||result.error||`LOCAL_EXIT_${result.exitCode??"UNKNOWN"}`)}});
    }catch(error){if(isSessionError(error)){await clearExpiredSessionAndWake(cfg,error);return {state:"WAIT_AUTH",taskId:id};}throw error;}
    await setActive(null);return {state:result.ok?"COMPLETED":"FAILED",taskId:id};
  }
  if(["ERROR","NOT_FOUND"].includes(String(host.state||""))){try{await api(cfg.appsScriptUrl,{action:"updateTask",sessionToken:token,taskId:id,status:"ERROR",patch:{error:String(host.error||host.state)}});}catch{};await setActive(null);return {state:String(host.state),taskId:id};}
...
async function pollCore(reason="alarm"){
  ...
  const listedTasks=Array.isArray(listed.tasks)?listed.tasks:[];const recovered=await recoverStale(cfg,token,listedTasks);const map=new Map();
  for(const t of [...recovered,...listedTasks]){if(taskType(t)!==ASYNC_TYPE)continue;if(!["READY","RETRY","ERROR"].includes(taskStatus(t)))continue;if(taskId(t))map.set(taskId(t),t);}
  const tasks=[...map.values()];if(!tasks.length)return {ok:true,reason,processed:0,asyncAvailable:0};
  const t=tasks[0],id=taskId(t);const profile=await chrome.identity.getProfileUserInfo({accountStatus:"ANY"}).catch(()=>({email:""}));
  ...
  const claimedTask=claimed.task||t;const started=await hostStart(claimedTask);await setActive({taskId:id,timeoutSeconds:timeoutSeconds(claimedTask),startedAtMs:Date.now(),hostState:started.state||"STARTED"});
