...
function Run([hashtable]$task,[switch]$WithReceipts,[string]$BindingStatus='READY_FOR_EXACT_INTERFACE_BINDING',[int]$Expected=0){
  ...
  if($WithReceipts){
    @{ok=$true;status=$BindingStatus}|ConvertTo-Json|Set-Content (Join-Path $dir 'CHATGPT_IMAGE_ADAPTER_BINDING_PLAN_V1.json') -Encoding UTF8
...
$base=@{taskId='T1';prompt='make a simple test image';sourceWorkflow='TEST';createdAt='2026-09-12T09:20:00+09:00'}
...
if($r.status -ne 'READY_FOR_EXACT_INTERFACE_CONFIRMATION' -or -not $r.automaticQueueIntake -or $r.automaticBrowserAction){throw 'QUEUE_PREP_EXACT_BINDING_FAIL'}
$r=Run ($base+@{flowQuotaState='BLOCKED';chatGptQuotaState='AVAILABLE';fallbackReason='FLOW_CREDIT_OR_QUOTA_BLOCKED'}) -WithReceipts -BindingStatus 'READY_FOR_ADAPTER_BINDING'
if($r.status -ne 'READY_FOR_EXACT_INTERFACE_CONFIRMATION'){throw 'QUEUE_PREP_LEGACY_BINDING_COMPAT_FAIL'}
