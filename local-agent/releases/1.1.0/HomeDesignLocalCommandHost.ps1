...
function Log([string]$m){
  Add-Content -LiteralPath $LogFile -Value "[$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')] $m" -Encoding UTF8
}
...
$listener.Start()
Log 'Local command host START 127.0.0.1:8765'
...
while($listener.IsListening){
  ...
  try{
    ...
    if($ctx.Request.HttpMethod -eq 'GET' -and $path -eq '/health'){
      Send-Json $ctx @{ok=$true;service='HomeDesign Local Command Host';version='1.1.0';time=(Get-Date).ToString('o')}
      continue
    }
    if($ctx.Request.HttpMethod -ne 'POST' -or $path -ne '/run'){
    ...
    if([string]$spec.repo -ne $AllowedRepo){throw 'repo not allowed'}
    if([string]$spec.branch -ne $AllowedBranch){throw 'branch not allowed'}
    if($AllowedScripts -notcontains [string]$spec.script){throw 'script not allowlisted'}

    $safeScript=[string]$spec.script
    if($safeScript -match '\.\.' -or $safeScript.StartsWith('/')){throw 'unsafe script path'}
    $rawUrl="https://raw.githubusercontent.com/$AllowedRepo/$AllowedBranch/$safeScript"
    $taskDir=Join-Path $ResultRoot ([string]$task.taskId)
    New-Item -ItemType Directory -Force -Path $taskDir|Out-Null
    $localScript=Join-Path $taskDir ([IO.Path]::GetFileName($safeScript))
    Invoke-WebRequest -UseBasicParsing -Uri $rawUrl -OutFile $localScript -TimeoutSec 60

    $args=@('-NoProfile','-ExecutionPolicy','Bypass','-File',$localScript)
    if($spec.args){
      foreach($p in $spec.args.PSObject.Properties){
        $name=[string]$p.Name
        if($name -notmatch '^[A-Za-z][A-Za-z0-9_]*$'){throw "unsafe arg name: $name"}
        $args += "-$name"
        $args += [string]$p.Value
      }
    }

    Log ("RUN task="+$task.taskId+" script="+$safeScript)
    ...
    Log ("DONE task="+$task.taskId+" exit="+$r.exitCode)
    Send-Json $ctx $result $(if($r.exitCode -eq 0){200}else{500})
  }catch{
    $msg=$_.Exception.Message
    Log ("ERROR "+$msg)
    if($ctx){try{Send-Json $ctx @{ok=$false;error=$msg;at=(Get-Date).ToString('o')} 500}catch{}}
  }
}
