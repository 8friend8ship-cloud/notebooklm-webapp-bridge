...
$ChatMarker='CHAT_AUDIO_RUN_20260830_2315'
...
$MarkerFile=Join-Path $Root 'AGENT_1.1.115_NATIVE_CHAT_STUDIO_AUDIO_START_RESULT.json'
...
if(Test-Path -LiteralPath $MarkerFile){
  try{
    ...
    if([bool]$prior.ok){[void](SaveCentral 'AGENT_1.1.115_NATIVE_CHAT_STUDIO_AUDIO_START_RESULT.json' $prior);$prior|ConvertTo-Json -Depth 80 -Compress;exit 0}
  }catch{}
}
$result=[ordered]@{ok=$false;action='AGENT_1.1.115_NATIVE_CHAT_STUDIO_AUDIO_START';version=$Version;targetUrl=$TargetUrl;chatMarker=$ChatMarker;startedAt=(Get-Date).ToString('o');exactTab=$false;chatAlreadyPresent=$false;chatTabClicked=$false;editorFound=$false;nativePromptInserted=$false;submitEnabled=$false;submitClicked=$false;chatVerified=$false;studioSelected=$false;audioSelected=$false;customizeClicked=$false;studioInstructionFilled=$false;generateClicked=$false;generationStarted=$false;normalChromeTouched=$false;bridgeChanged=$false;oauthChanged=$false;scopeChanged=$false;stage='START';error='';centralPath=''}
$w=$null
try{
  ...
  if($present){$result.chatAlreadyPresent=$true;$result.chatVerified=$true}else{
    ...
    $result.stage='CHAT_NATIVE_FILL'
    $e=Eval $w ([ref]$q) "(() => {const v=e=>{if(!(e instanceof HTMLElement))return false;const r=e.getBoundingClientRect(),s=getComputedStyle(e);return r.width>4&&r.height>4&&s.display!=='none'&&s.visibility!=='hidden'};const a=[...document.querySelectorAll('textarea,[contenteditable=true]')].filter(v);const e=a.find(x=>{const p=String(x.getAttribute('placeholder')||'');const ar=String(x.getAttribute('aria-label')||'');return p.includes('\uC9C8\uBB38')||p.includes('\uCC3D\uC791')||ar.includes('\uCFFC\uB9AC')||/ask|create/i.test(p)})||a[0];if(!e)return{ok:false,count:a.length};const r=e.getBoundingClientRect();return{ok:true,x:r.left+r.width/2,y:r.top+r.height/2}})()"
    if(-not$e.ok){throw 'CHAT_EDITOR_NOT_FOUND'};$result.editorFound=$true;Click $w ([ref]$q) ([double]$e.x) ([double]$e.y)
    [void](Send $w ([ref]$q) 'Input.dispatchKeyEvent' @{type='keyDown';key='a';code='KeyA';modifiers=2})
    [void](Send $w ([ref]$q) 'Input.dispatchKeyEvent' @{type='keyUp';key='a';code='KeyA';modifiers=2})
    [void](Send $w ([ref]$q) 'Input.dispatchKeyEvent' @{type='keyDown';key='Backspace';code='Backspace';windowsVirtualKeyCode=8})
    [void](Send $w ([ref]$q) 'Input.dispatchKeyEvent' @{type='keyUp';key='Backspace';code='Backspace';windowsVirtualKeyCode=8})
    [void](Send $w ([ref]$q) 'Input.insertText' @{text=$Prompt});$result.nativePromptInserted=$true
...
finally{if($w){try{$w.Dispose()}catch{}};$result.completedAt=(Get-Date).ToString('o');$result.centralPath=SaveCentral 'AGENT_1.1.115_NATIVE_CHAT_STUDIO_AUDIO_START_RESULT.json' $result}
