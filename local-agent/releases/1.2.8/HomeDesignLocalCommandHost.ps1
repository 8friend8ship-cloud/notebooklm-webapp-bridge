...
$Source=Join-Path $Root 'HomeDesignLocalCommandHost-1.2.0-source.ps1';$Patched=Join-Path $Root 'HomeDesignLocalCommandHost-1.2.8-patched.ps1'
$Base='https://raw.githubusercontent.com/8friend8ship-cloud/notebooklm-webapp-bridge/main/local-agent/releases/1.2.0/HomeDesignLocalCommandHost.ps1';$Expected='3cee6a57aa2fd2ab9bcf04f275f4c95296bba247'
...
$OldVersion="`$HostVersion='1.2.0'";$NewVersion="`$HostVersion='1.2.8'";if(-not $Code.Contains($OldVersion)){throw 'HOST_VERSION_PATCH_TARGET_MISSING'};$Code=$Code.Replace($OldVersion,$NewVersion)
...
$ContentOsAnchor="[pscustomobject]@{repo='8friend8ship-cloud/notebooklm-webapp-bridge';branch='main';scripts=$NotebookAllowNew}"
$ContentOsRule=$ContentOsAnchor+",`r`n  [pscustomobject]@{repo='8friend8ship-cloud/contents-os-git';branch='main';scripts=@('tools/Switch-ContentOS-VercelGit.ps1','tools/Repair-ContentOS-DriveCacheAppsScript.ps1')}"
