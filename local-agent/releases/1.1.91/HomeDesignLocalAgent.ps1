...
$PatchedHelper=Join-Path $Root 'RunNotebookLMExactOrderedResearchAudioV2-returnfix.ps1'
...
$Stdout=Join-Path $Root 'AGENT_1.1.91.stdout.log'
$Stderr=Join-Path $Root 'AGENT_1.1.91.stderr.log'
...
$result=[ordered]@{ok=$false;action='AGENT_1.1.91_EXACT_ORDER_RETURNFIX';version=$Version;startedAt=(Get-Date).ToString('o');sourceHelperSha=$ExpectedSourceSha;sourceFetch='';patchApplied=$false;patchedHelperSha='';helperExit=$null;stage='START';notebookUrl='';titleVerified=$false;sourceVerified=$false;sourceCount=0;chatSubmitted=$false;studioSelected=$false;generateClicked=$false;artifactReady=$false;bridgeChanged=$false;normalChromeTouched=$false;oauthChanged=$false;scopeChanged=$false;error='';centralPath=''}
...
try{
  ...
  $old='return$z.result'
  $new='return $z.result'
  if(-not $txt.Contains($old)){throw 'RETURN_RUNTIME_PATCH_ANCHOR_NOT_FOUND'}
  $patched=$txt.Replace($old,$new)
  if($patched.Contains($old)){throw 'RETURN_RUNTIME_PATCH_NOT_COMPLETE'}
  ...
  if(-not $result.ok -and -not $result.error){$result.error='EXACT_ORDER_RETURNFIX_GATE_FAILED'}
}catch{$result.error=$_.Exception.Message}
finally{if($held){try{$m.ReleaseMutex()}catch{}};$m.Dispose();$result.completedAt=(Get-Date).ToString('o');$result.centralPath=SaveCentral 'AGENT_1.1.91_EXACT_ORDER_RETURNFIX_RESULT.json' $result}
