...
$Version='1.1.142-image-task203-host130-portowner'
...
function PortOwnerPids{
  $ids=@()
  try{$ids+=@(Get-NetTCPConnection -LocalPort 8765 -State Listen -ErrorAction Stop|ForEach-Object{[int]$_.OwningProcess})}catch{}
  if($ids.Count-eq0){
    try{foreach($line in @(netstat -ano -p tcp 2>$null)){if($line -match ':8765\s+.*LISTENING\s+(\d+)\s*$'){$ids+=[int]$matches[1]}}}catch{}
  }
  @($ids|Where-Object{$_ -gt 0 -and $_ -ne $PID}|Sort-Object -Unique)
}
$r=[ordered]@{ok=$false;action='IMAGE_TASK203_HOST130_PORT_OWNER_RECOVERY_AND_RUN';version=$Version;targetHost='1.3.0';hostBaseSha=$HostBaseSha;hostPatchedSha='';repairCommit=$RepairCommit;repairSha=$RepairSha;childSha=$ChildSha;oldHealth=$null;newHealth=$null;oldHostPids=@();oldHostProcess=@();portOwnersBefore=@();portOwnersAfterStop=@();newHostPid=$null;childExit=$null;stage='START';error='';duplicateTaskCreated=$false;newAppsScriptProject=$false;newDeployment=$false;newTrigger=$false;oauthChanged=$false;scopeChanged=$false;normalChromeTouched=$false;generateClicked=$false;creditSpend=$false;startedAt=(Get-Date).ToString('o')}
try{
  $r.oldHealth=Health
...
'@
  ...
  $r.stage='STOP_EXACT_PORT8765_OWNER'
  $owners=@(PortOwnerPids);$r.portOwnersBefore=$owners
  if($owners.Count-gt0){
    if(-not $r.oldHealth -or [string]$r.oldHealth.service -ne 'HomeDesign Local Command Host'){throw('PORT8765_OWNER_PRESENT_BUT_HEALTH_NOT_HOMEDESIGN:'+($owners -join ','))}
    $r.oldHostPids=$owners
    foreach($p in $owners){
      try{$proc=Get-CimInstance Win32_Process -Filter ('ProcessId='+$p) -ErrorAction SilentlyContinue;if($proc){$r.oldHostProcess+=([ordered]@{pid=$p;name=[string]$proc.Name;commandLine=[string]$proc.CommandLine})}}catch{}
      try{Stop-Process -Id $p -Force -ErrorAction Stop}catch{throw('PORT8765_STOP_FAILED_PID_'+$p+':'+$_.Exception.Message)}
    }
  }
  for($i=0;$i-lt20;$i++){Start-Sleep -Milliseconds 250;if(@(PortOwnerPids).Count-eq0){break}}
  $r.portOwnersAfterStop=@(PortOwnerPids)
  if($r.portOwnersAfterStop.Count-ne0){throw('PORT8765_STILL_LISTENING_AFTER_STOP:'+($r.portOwnersAfterStop -join ','))}
  $r.stage='START_PATCHED_HOST130'
  ...
  for($i=0;$i-lt40;$i++){Start-Sleep -Milliseconds 500;$h=Health;if($h -and [string]$h.version -eq '1.3.0' -and [bool]$h.asyncJobs){$r.newHealth=$h;break}}
  if(-not $r.newHealth){throw('PATCHED_HOST130_START_HEALTH_FAILED:'+((Health|ConvertTo-Json -Compress -Depth 10))+':PORTOWNERS='+(@(PortOwnerPids)-join ','))}
...
}catch{$r.error=$_.Exception.Message;$r.stage='ERROR'}finally{$r.completedAt=(Get-Date).ToString('o');Save 'IMAGE_TASK203_HOST130_PORTOWNER_1.1.142.json' $r}
