...
$Version='1.1.146-bootstrap-loop-refresh'
...
$Receipt='BOOTSTRAP_LOOP_REFRESH_1.1.146.json'
...
try{
  ...
  $tmp=$Bootstrap+'.refresh146';[IO.File]::WriteAllBytes($tmp,[Convert]::FromBase64String(([string]$meta.content-replace'\s','')))
  ...
  $parentPid=[int]$self.ParentProcessId;$parent=Get-CimInstance Win32_Process -Filter ('ProcessId='+$parentPid) -ErrorAction Stop
  ...
  $helper=Join-Path $Root 'RESTART_AGENTBOOTSTRAP_LOOP_146.ps1'
  $helperText=@'
...
try{
  ...
  if($p -and [string]$p.CommandLine -match '(?i)AgentBootstrap\.ps1' -and [string]$p.CommandLine -match '(?i)(?:^|\s)-Loop(?:\s|$)'){Stop-Process -Id $OldPid -Force -ErrorAction SilentlyContinue}
}catch{}
...
try{[ordered]@{ok=$true;action='AGENTBOOTSTRAP_LOOP_RESTART_HELPER';oldPid=$OldPid;bootstrap=$Bootstrap;at=(Get-Date).ToString('o')}|ConvertTo-Json|Set-Content -LiteralPath (Join-Path $Root 'BOOTSTRAP_LOOP_RESTART_HELPER_146.json') -Encoding UTF8}catch{}
'@
