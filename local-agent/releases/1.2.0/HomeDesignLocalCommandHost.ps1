...
function Start-AsyncTask($Task){
  ...
  $rawUrl="https://raw.githubusercontent.com/$($rule.repo)/$($rule.branch)/$safeScript";$localScript=Join-Path $taskDir ([IO.Path]::GetFileName($safeScript));Invoke-WebRequest -UseBasicParsing -Uri $rawUrl -OutFile $localScript -TimeoutSec 60
