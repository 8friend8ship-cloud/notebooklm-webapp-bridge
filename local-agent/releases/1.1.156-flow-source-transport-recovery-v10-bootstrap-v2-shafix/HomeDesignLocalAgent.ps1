...
$Version='1.1.156-flow-source-transport-recovery-v10-bootstrap-v2-shafix'
$Repo='8friend8ship-cloud/notebooklm-webapp-bridge'
$Root=Join-Path $env:LOCALAPPDATA 'HomeDesignAutomationV7\LocalAgent'
$Watchdog=Join-Path $Root 'HomeDesignLocalWatchdog.ps1'
$Bootstrap=Join-Path $Root 'AgentBootstrap.ps1'
$Receipt='FLOW_SOURCE_TRANSPORT_RECOVERY_V10_BOOTSTRAP_V2_1.1.156.json'
...
function RawBytes([string]$Path){$u='https://raw.githubusercontent.com/'+$Repo+'/main/'+$Path+'?cb='+[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds();$wc=New-Object Net.WebClient;try{$wc.Headers['User-Agent']='HomeDesign-Flow-Transport-Recovery-1.1.156';return $wc.DownloadData($u)}finally{$wc.Dispose()}}
...
function ReplaceVerified([string]$RepoPath,[string]$Dest,[string]$Expected,[string]$BackupSuffix){$b=RawBytes $RepoPath;if(-not$b-or$b.Length-eq0){throw ('RAW_EMPTY:'+ $RepoPath)};$sha=(GitBlobSha1Bytes $b).ToLowerInvariant();if($sha-ne$Expected.ToLowerInvariant()){throw ('RAW_BLOB_MISMATCH:'+ $RepoPath+':'+$sha)};$before='';if(Test-Path -LiteralPath $Dest -PathType Leaf){$before=(GitBlobSha1 $Dest).ToLowerInvariant();if($before-ne$Expected.ToLowerInvariant()){Copy-Item -LiteralPath $Dest -Destination ($Dest+$BackupSuffix) -Force}};if($before-ne$Expected.ToLowerInvariant()){$tmp=$Dest+'.recovery156';[IO.File]::WriteAllBytes($tmp,$b);if((GitBlobSha1 $tmp).ToLowerInvariant()-ne$Expected.ToLowerInvariant()){Remove-Item $tmp -Force -ErrorAction SilentlyContinue;throw ('LOCAL_VERIFY_FAILED:'+ $RepoPath)};Move-Item $tmp $Dest -Force};[pscustomobject]@{before=$before;after=(GitBlobSha1 $Dest).ToLowerInvariant();changed=($before-ne$Expected.ToLowerInvariant())}}

$r=[ordered]@{ok=$false;action='FLOW_SOURCE_TRANSPORT_RECOVERY_V10_BOOTSTRAP_V2_SHAFIX';version=$Version;stage='START';watchdogExpected=$ExpectedWatchdog;bootstrapExpected=$ExpectedBootstrap;watchdogBefore='';watchdogAfter='';watchdogChanged=$false;bootstrapBefore='';bootstrapAfter='';bootstrapChanged=$false;bootstrapProcessesBefore=0;bootstrapProcessesStopped=0;bootstrapProcessesAfter=0;bootstrapRestarted=$false;normalChromeTouched=$false;flowGenerateClicked=$false;hostTouched=$false;oauthChanged=$false;scopeChanged=$false;creditSpend=$false;error='';startedAt=(Get-Date).ToString('o');completedAt=''}
try{
  $stamp='.backup_1.1.156_'+(Get-Date -Format 'yyyyMMdd_HHmmss')
