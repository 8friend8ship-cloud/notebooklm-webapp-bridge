...
$PatchedHelper=Join-Path $Root 'RunNotebookLMExactOrderedResearchAudioV4-tabfix.ps1'
...
$Stdout=Join-Path $Root 'AGENT_1.1.93.stdout.log';$Stderr=Join-Path $Root 'AGENT_1.1.93.stderr.log'
...
try{
 ...
 $sourceOld="s==='\\uCD9C\\uCC98'||s.toLowerCase()==='sources'";$sourceNew="s.includes('\\uCD9C\\uCC98')||s.toLowerCase().includes('sources')"
 $chatOld="s==='\\uCC44\\uD305'||s.toLowerCase()==='chat'";$chatNew="s.includes('\\uCC44\\uD305')||s.toLowerCase().includes('chat')"
 $studioOld="s==='\\uC2A4\\uD29C\\uB514\\uC624'||s.toLowerCase()==='studio'";$studioNew="s.includes('\\uC2A4\\uD29C\\uB514\\uC624')||s.toLowerCase().includes('studio')"
 foreach($anchor in @('return$z.result',"throw'",$sourceOld,$chatOld,$studioOld)){if(-not $txt.Contains($anchor)){throw ('TABFIX_PATCH_ANCHOR_NOT_FOUND '+$anchor)}}
 ...
 foreach($bad in @('return$z.result',"throw'",$sourceOld,$chatOld,$studioOld)){if($patched.Contains($bad)){throw ('TABFIX_PATCH_NOT_COMPLETE '+$bad)}}
 $patched|Set-Content -LiteralPath $PatchedHelper -Encoding UTF8;$result.patchApplied=$true;$result.patchedHelperSha=GitBlobSha1 $PatchedHelper
 $tokens=$null;$parseErrors=$null;[void][System.Management.Automation.Language.Parser]::ParseFile($PatchedHelper,[ref]$tokens,[ref]$parseErrors);if($parseErrors.Count -gt 0){throw ('PATCHED_HELPER_PARSE_FAIL '+(($parseErrors|ForEach-Object{$_.Message})-join' | '))}
 ...
 $payload=$null;foreach($line in @($out -split "`r?`n"|Where-Object{$_.Trim()}|Select-Object -Last 50)){try{$j=$line|ConvertFrom-Json;if($j){$payload=$j}}catch{}};if(-not $payload){throw ('HELPER_RESULT_JSON_NOT_FOUND '+$out.Trim())}
 $result.stage=[string]$payload.stage;$result.notebookUrl=[string]$payload.notebookUrl;$result.titleVerified=[bool]$payload.titleVerified;$result.sourceVerified=[bool]$payload.sourceVerified;$result.sourceCount=[int]$payload.sourceCount;$result.chatSubmitted=[bool]$payload.chatSubmitted;$result.studioSelected=[bool]$payload.studioSelected;$result.generateClicked=[bool]$payload.generateClicked;$result.artifactReady=[bool]$payload.artifactReady;if($payload.error){$result.error=[string]$payload.error}
 $result.ok=([bool]$payload.ok -and $result.titleVerified -and $result.sourceVerified -and $result.sourceCount -gt 0 -and $result.chatSubmitted -and $result.studioSelected -and $result.generateClicked -and $result.artifactReady);if(-not $result.ok -and -not $result.error){$result.error='EXACT_ORDER_TABFIX_GATE_FAILED'}
}catch{$result.error=$_.Exception.Message}
finally{if($held){try{$m.ReleaseMutex()}catch{}};$m.Dispose();$result.completedAt=(Get-Date).ToString('o');$result.centralPath=SaveCentral 'AGENT_1.1.93_EXACT_ORDER_TABFIX_RESULT.json' $result}
