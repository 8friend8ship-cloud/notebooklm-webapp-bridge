...
$LaneVersion='drive-mirror-0.1.1-volatile-filter'
...
function FetchRepoBytes([string]$Path){
  try{$u='https://raw.githubusercontent.com/'+$Repo+'/main/'+$Path+'?cb='+[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds();$wc=New-Object Net.WebClient;try{$wc.Headers['User-Agent']='HomeDesign-Drive-Mirror-Lane-0.1.1';return $wc.DownloadData($u)}finally{$wc.Dispose()}}catch{}
  $h=@{'User-Agent'='HomeDesign-Drive-Mirror-Lane-0.1.1';'Accept'='application/vnd.github+json'}
