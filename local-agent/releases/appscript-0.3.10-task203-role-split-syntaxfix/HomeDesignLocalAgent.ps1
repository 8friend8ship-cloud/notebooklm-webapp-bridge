...
$Version='appscript-0.3.10-task203-role-split-syntaxfix'
...
$Receipt='APPSCRIPT_TASK203_ROLE_SPLIT_SYNTAXFIX_0.3.10.json'
...
try{
  ...
  try{$wc.Headers['User-Agent']='HomeDesign-Task203-SyntaxFix';$b=$wc.DownloadData($url)}finally{$wc.Dispose()}
  ...
  $tmp=Join-Path $env:TEMP ('task203-0.3.10-patched-'+(Get-Date -Format 'yyyyMMdd_HHmmss_fff')+'.ps1')
