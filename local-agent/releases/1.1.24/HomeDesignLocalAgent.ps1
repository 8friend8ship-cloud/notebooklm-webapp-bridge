...
$RecoveryFile=Join-Path $Root 'AGENT_1.1.24_RECOVERY.json'
$HostUrl='https://raw.githubusercontent.com/8friend8ship-cloud/notebooklm-webapp-bridge/main/local-agent/releases/1.2.3/HomeDesignLocalCommandHost.ps1'
$HostExpected='bf960b1f786b7f3e6c8d3297bc2aa8547b3983b7'
...
function StartHost123{
  ...
  $deadline=(Get-Date).AddSeconds(20);do{Start-Sleep -Milliseconds 500;$h=HostHealth;if($h -and [bool]$h.ok -and [string]$h.version -eq '1.2.3' -and [bool]$h.asyncJobs){return $h}}while((Get-Date)-lt $deadline)
  throw 'HOST_1.2.3_START_FAILED'
}
...
function PromoteState{
  ...
  if($s.PSObject.Properties.Name -contains 'agentMode'){$s.agentMode='HOST_123_CONTENTOS_RELINK_ALLOWLIST_1.1.24'}else{$s|Add-Member -NotePropertyName agentMode -NotePropertyValue 'HOST_123_CONTENTOS_RELINK_ALLOWLIST_1.1.24'}
  if($s.PSObject.Properties.Name -contains 'commandHostVersion'){$s.commandHostVersion='1.2.3'}else{$s|Add-Member -NotePropertyName commandHostVersion -NotePropertyValue '1.2.3'}
...
$r=[ordered]@{action='AGENT_1.1.24_HOST_123_CONTENTOS_RELINK_ALLOWLIST';startedAt=(Get-Date).ToString('o');recovery122Exit=$null;hostBlobExpected=$HostExpected;hostRefreshed=$false;hostHealthy=$false;statePromoted=$false;ok=$false;error=''}
try{
  ...
  StopHost;Start-Sleep -Milliseconds 700;$h=StartHost123;$r.hostHealthy=[bool]$h.ok
  $h2=HostHealth;if(-not($h2 -and [bool]$h2.ok -and [string]$h2.version -eq '1.2.3' -and [bool]$h2.asyncJobs)){throw 'HOST_1.2.3_NOT_HEALTHY'}
