...
$Version='1.1.140-image-task203-host130'
...
$HostSha='274aef614763cfe1d886fd16c5641e0b64ab6176'
...
$r=[ordered]@{ok=$false;action='IMAGE_TASK203_HOST130_RECOVERY_AND_RUN';version=$Version;targetHost='1.3.0';hostSha=$HostSha;childSha=$ChildSha;oldHealth=$null;newHealth=$null;oldHostPids=@();newHostPid=$null;childExit=$null;stage='START';error='';duplicateTaskCreated=$false;newAppsScriptProject=$false;newDeployment=$false;newTrigger=$false;oauthChanged=$false;scopeChanged=$false;normalChromeTouched=$false;generateClicked=$false;creditSpend=$false;startedAt=(Get-Date).ToString('o')}
try{
  $r.oldHealth=Health
  if(-not($r.oldHealth -and [string]$r.oldHealth.version -eq '1.3.0' -and [bool]$r.oldHealth.asyncJobs)){
    $r.stage='FETCH_HOST130_RAW'
    [void](RawVerified $HostCommit $HostPathRepo $HostPath $HostSha)
    $r.stage='RESTART_HOST130'
    ...
    for($i=0;$i-lt20;$i++){Start-Sleep -Milliseconds 500;$h=Health;if($h -and [string]$h.version -eq '1.3.0' -and [bool]$h.asyncJobs){$r.newHealth=$h;break}}
    if(-not $r.newHealth){throw('HOST130_START_HEALTH_FAILED:'+((Health|ConvertTo-Json -Compress -Depth 10)))}
  }else{$r.newHealth=$r.oldHealth}
...
}catch{$r.error=$_.Exception.Message;$r.stage='ERROR'}finally{$r.completedAt=(Get-Date).ToString('o');Save 'IMAGE_TASK203_HOST130_RECOVERY_1.1.140.json' $r}
