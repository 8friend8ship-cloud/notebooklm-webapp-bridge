...
function AgeSec($ProcessRow){try{$d=$ProcessRow.CreationDate;if(-not($d -is [datetime])){$d=[Management.ManagementDateTimeConverter]::ToDateTime([string]$d)};return [Math]::Max(0,((Get-Date)-$d).TotalSeconds)}catch{return 0}}
...
function ClearStaleGovernorReadback(){
  $killed=@()
  foreach($processRow in (Proc 'RunChromeGovernorReadback.ps1')){
    if((AgeSec $processRow)-gt 150){KillTree -ProcessId ([int]$processRow.ProcessId);$killed+=('RunChromeGovernorReadback.ps1:'+$processRow.ProcessId)}
  }
  if($killed.Count -gt 0){Start-Sleep -Seconds 2}
  return $killed
}
function StartHidden([string]$File,[string[]]$Extra){$a=@('-NoProfile','-ExecutionPolicy','Bypass','-WindowStyle','Hidden','-File',"`"$File`"")+$Extra;Start-Process powershell.exe -ArgumentList $a -WindowStyle Hidden;Start-Sleep -Milliseconds 700}
function TestHostHealth(){
  try{$h=Invoke-RestMethod -Uri 'http://127.0.0.1:8765/health' -Method Get -TimeoutSec 3;return ([bool]$h.ok -and [string]$h.version -eq $HostVersion)}catch{return $false}
}
...
function WriteState($Killed){
  $state=@{};if(Test-Path $StateFile){try{$o=Get-Content $StateFile -Raw -Encoding UTF8|ConvertFrom-Json;foreach($prop in $o.PSObject.Properties){$state[$prop.Name]=$prop.Value}}catch{}}
  $state.agentVersion=$AgentVersion;$state.commandHostVersion=$HostVersion;$state.commandHostRunning=(TestHostHealth);$state.commandHostUrl='http://127.0.0.1:8765';$state.commandHostTransport='TcpListener';$state.taskTimeoutEnforced=$true;$state.staleGovernorReadbackKilled=@($Killed);$state.updatedAt=(Get-Date).ToString('o');$state|ConvertTo-Json -Depth 20|Set-Content $StateFile -Encoding UTF8
}

$HostUrl='https://raw.githubusercontent.com/8friend8ship-cloud/notebooklm-webapp-bridge/main/local-agent/releases/1.1.4/HomeDesignLocalCommandHost.ps1'
...
$hostChanged=EnsureFile $HostUrl $HostFile '4dbf4b37eac5d5750dea0759edb5cfb3fb10fc8b'
[void](EnsureFile $LegacyUrl $LegacyFile '6bf74c7b3a58e6f2bd118d11d906dda031796e57')
$hostHealthy=TestHostHealth
if($hostChanged -or -not $hostHealthy){
  foreach($processRow in (Proc 'HomeDesignLocalCommandHost.ps1')){KillTree -ProcessId ([int]$processRow.ProcessId)}
  Start-Sleep -Milliseconds 750
  StartHidden $HostFile @()
  $deadline=(Get-Date).AddSeconds(10);do{Start-Sleep -Milliseconds 500;$hostHealthy=TestHostHealth}while(-not $hostHealthy -and (Get-Date)-lt $deadline)
}
RefreshGovernor
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $LegacyFile
if(-not(TestHostHealth)){foreach($processRow in (Proc 'HomeDesignLocalCommandHost.ps1')){KillTree -ProcessId ([int]$processRow.ProcessId)};StartHidden $HostFile @()}
WriteState $killed
if(-not(TestHostHealth)){throw 'Local Command Host health failed after v1.1.4 start.'}
