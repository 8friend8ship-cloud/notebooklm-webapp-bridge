...
function Invoke-PowerShellCaptured([string]$Path, [string[]]$Args) {
  $allArgs = @('-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',$Path) + $Args
  $output = & powershell.exe @allArgs 2>&1 | Out-String
  return [ordered]@{exitCode=$LASTEXITCODE;stdout=$output.Trim()}
}
...
$receipt = [ordered]@{
  ...
  action = 'AGENT_1.1.47_POWER_CONTINUITY_SELF_HEAL'
...
if ($runtimeRoot) {
  Save-Json (Join-Path $runtimeRoot 'AGENT_1.1.47_POWER_CONTINUITY_SELF_HEAL.json') $receipt
...
try {
  ...
  $stateObject | Add-Member agentMode 'POWER_CONTINUITY_SELF_HEAL_1.1.47' -Force
