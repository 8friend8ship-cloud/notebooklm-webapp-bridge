...
function ClearStaleAllowlistedTasks(){
  $killed=@()
  foreach($rule in @(
    @{needle='Run-AnimationTestDeploymentE2E.ps1';age=660},
    @{needle='RunChromeGovernorReadback.ps1';age=180}
  )){
    foreach($p in (Proc $rule.needle)){
      if((AgeSec $p)-gt [double]$rule.age){try{Stop-Process -Id $p.ProcessId -Force -ErrorAction Stop;$killed+=($rule.needle+':'+$p.ProcessId)}catch{}}
    }
  }
...
function RefreshGovernor(){
  ...
  foreach($name in @('ChromeExtensionGovernor.ps1','SHOW_CHROME_EXTENSION_GOVERNOR_STATUS.ps1','GovernorDriveSync.ps1')){$dst=Join-Path $GovernorRoot $name;$tmp=$dst+'.download';Invoke-WebRequest -UseBasicParsing -Uri "$base/$name" -OutFile $tmp -TimeoutSec 60;$changed=(-not(Test-Path $dst));if(-not $changed){$changed=(Get-FileHash $dst -Algorithm SHA256).Hash -ne (Get-FileHash $tmp -Algorithm SHA256).Hash};if($changed){Move-Item $tmp $dst -Force}else{Remove-Item $tmp -Force}}
...
function WriteState($Killed){
  ...
  $state.agentVersion=$AgentVersion;$state.commandHostVersion='1.1.2';$state.commandHostRunning=((Proc 'HomeDesignLocalCommandHost.ps1').Count -gt 0);$state.commandHostUrl='http://127.0.0.1:8765';$state.taskTimeoutEnforced=$true;$state.videoRendererAllowlisted=$true;$state.staleAllowlistedTasksKilled=@($Killed);$state.updatedAt=(Get-Date).ToString('o');$state|ConvertTo-Json -Depth 20|Set-Content $StateFile -Encoding UTF8
}
...
$killed=ClearStaleAllowlistedTasks
...
if($hostChanged){foreach($p in (Proc 'HomeDesignLocalCommandHost.ps1')){Stop-Process -Id $p.ProcessId -Force -ErrorAction SilentlyContinue};Start-Sleep -Milliseconds 750}
RefreshGovernor
