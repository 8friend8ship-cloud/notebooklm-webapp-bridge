...
try{
  if(Test-Path -LiteralPath $Marker){try{$mj=Get-Content -LiteralPath $Marker -Raw -Encoding UTF8|ConvertFrom-Json;if([bool]$mj.done -and [string]$mj.bootstrapSha -eq $ExpectedBootstrapSha){$r.ok=$true;$r.alreadyDone=$true;$r.bootstrapActualSha=[string]$mj.bootstrapSha;throw [System.Management.Automation.HaltCommandException]::new('SENTINEL_ALREADY_DONE')}}catch [System.Management.Automation.HaltCommandException]{throw}catch{}}
...
}catch [System.Management.Automation.HaltCommandException]{if(-not$r.ok){$r.error=$_.Exception.Message}}catch{$r.error=$_.Exception.Message}
