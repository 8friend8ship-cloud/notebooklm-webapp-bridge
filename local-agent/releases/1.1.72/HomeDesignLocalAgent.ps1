...
$AttemptMarker=Join-Path $Root 'NOTEBOOKLM_ROLLBACK_0275_FRESH_CREATE_1.1.72.attempted'
$ResultPath=Join-Path $Root 'NOTEBOOKLM_ROLLBACK_0275_FRESH_CREATE_1.1.72.json'
...
function PublishReceipt($Obj){
  ...
  if($central){try{SaveJson (Join-Path $central 'Runtime_Readback\NotebookLM\NOTEBOOKLM_ROLLBACK_0275_FRESH_CREATE_1.1.72.json') $Obj}catch{}}
}
