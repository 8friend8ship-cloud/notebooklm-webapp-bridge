...
$Prev=Join-Path $Root 'HomeDesignLocalAgent-1.1.55.ps1'
$PrevBlob='90acc698096e3d94f2f6a695410b8344f79c60ab'
...
function ScanChromeExtensions{
  $userData=Join-Path $env:LOCALAPPDATA 'Google\Chrome\User Data';$out=@();if(-not(Test-Path -LiteralPath $userData)){return @()}
  foreach($profile in @(Get-ChildItem -LiteralPath $userData -Directory -ErrorAction SilentlyContinue|Where-Object{$_.Name -eq 'Default' -or $_.Name -like 'Profile *'})){
    $pref=ReadJson (Join-Path $profile.FullName 'Preferences');$settings=$null;if($pref -and $pref.extensions -and $pref.extensions.settings){$settings=$pref.extensions.settings}
    $extRoot=Join-Path $profile.FullName 'Extensions';if(-not(Test-Path -LiteralPath $extRoot)){continue}
    foreach($idDir in @(Get-ChildItem -LiteralPath $extRoot -Directory -ErrorAction SilentlyContinue)){
      $verDir=Get-ChildItem -LiteralPath $idDir.FullName -Directory -ErrorAction SilentlyContinue|Sort-Object Name -Descending|Select-Object -First 1;if(-not $verDir){continue}
      $m=ReadJson (Join-Path $verDir.FullName 'manifest.json');if(-not $m){continue};$name=ResolveManifestName $m $verDir.FullName;$enabled=$null;$stateValue=$null
      if($settings -and ($settings.PSObject.Properties.Name -contains $idDir.Name)){$entry=$settings.($idDir.Name);if($entry.PSObject.Properties.Name -contains 'state'){$stateValue=$entry.state;try{$enabled=([int]$stateValue -eq 1)}catch{}}}
      $out += [pscustomobject]@{profile=$profile.Name;id=$idDir.Name;name=$name;version=[string]$m.version;enabled=$enabled;state=$stateValue;path=$verDir.FullName}
    }
  }
...
try{$prevSha=FetchBlob $PrevBlob $Prev;$p=Start-Process powershell.exe -ArgumentList @('-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',"`"$Prev`"") -WindowStyle Hidden -PassThru;$p.WaitForExit(90000)|Out-Null;if($p.HasExited){$prevExit=$p.ExitCode}}catch{$errors+=('PREVIOUS_AGENT_RECOVERY:'+$_.Exception.Message)}
...
$ok=[bool]($best -and $best.enabled -eq $true)
$central=FindCentralRoot
$receipt=[ordered]@{ok=$ok;action='CHATGPT_IMAGE_AUTO_CURRENT_RUNTIME_INVENTORY';agentVersion=$AgentVersion;status=$status;extension=$(if($best){$best}else{$null});matchCount=@($matches).Count;allExtensionCount=@($inventory).Count;previousAgentBlob=$prevSha;previousAgentExit=$prevExit;normalChromeTouched=$false;dedicatedChromeTouched=$false;flowTouched=$false;newOAuth=$false;newProject=$false;newDeployment=$false;newTrigger=$false;generateClicked=$false;creditSpend=$false;errors=$errors;at=(Get-Date).ToString('o')}
SaveJson $LocalReadback $receipt
if($central){try{$dest=Join-Path $central 'Runtime_Readback\CHROME\CHATGPT_IMAGE_AUTO_LIVE_READBACK.json';SaveJson $dest $receipt}catch{$errors+=('CENTRAL_READBACK_WRITE:'+$_.Exception.Message)}}
try{$s=ReadJson $State;if(-not $s){$s=[pscustomobject]@{}};$s|Add-Member agentVersion $AgentVersion -Force;$s|Add-Member agentMode 'CHATGPT_IMAGE_AUTO_READONLY_RECOVERY' -Force;$s|Add-Member status $status -Force;$s|Add-Member updatedAt ((Get-Date).ToString('o')) -Force;SaveJson $State $s}catch{}
