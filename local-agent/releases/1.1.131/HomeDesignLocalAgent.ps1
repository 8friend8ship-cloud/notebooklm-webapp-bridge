...
function Save($o){$j=$o|ConvertTo-Json -Depth 60;$j|Set-Content -LiteralPath $Marker -Encoding UTF8;try{$c=FindCentral;if($c){$d=Join-Path $c 'Runtime_Readback';New-Item -ItemType Directory -Force -Path $d|Out-Null;$j|Set-Content -LiteralPath (Join-Path $d $Receipt) -Encoding UTF8}}catch{}}
...
try{
  $r.stage='SOURCE_VERIFY';if(-not(Test-Path -LiteralPath $Source -PathType Leaf)){throw 'VERIFIED_AUDIO_SOURCE_NOT_FOUND'};$s=Get-Item -LiteralPath $Source;$ssh=Sha $Source;if([int64]$s.Length-ne[int64]$ExpectedBytes){throw ('SOURCE_SIZE_MISMATCH actual='+$s.Length)};if($ssh-ne$ExpectedSha){throw ('SOURCE_SHA_MISMATCH actual='+$ssh)};$r.sourceVerified=$true
  $r.stage='CAPTURE_INBOX_COPY';New-Item -ItemType Directory -Force -Path $Inbox|Out-Null;if(Test-Path -LiteralPath $Dest -PathType Leaf){$old=Get-Item -LiteralPath $Dest;$oldSha=Sha $Dest;if($old.Length-eq$s.Length-and$oldSha-eq$ExpectedSha){$r.copied=$true}else{Remove-Item -LiteralPath $Dest -Force}}
  if(-not$r.copied){Copy-Item -LiteralPath $Source -Destination $Dest -Force;$r.copied=$true}
  $d=Get-Item -LiteralPath $Dest;$dsha=Sha $Dest;$r.destinationBytes=[int64]$d.Length;$r.destinationSha256=$dsha;$sourceAfter=Sha $Source;$r.sourceImmutableVerified=($sourceAfter-eq$ExpectedSha);if($d.Length-ne$s.Length-or$dsha-ne$ExpectedSha-or-not$r.sourceImmutableVerified){throw 'CAPTURE_INBOX_COPY_INTEGRITY_FAILED'}
  try{$q=& schtasks.exe /Query /TN $r.scheduledTask /FO LIST /V 2>$null|Out-String;if($LASTEXITCODE-eq0){$r.scheduledTaskExists=$true;$r.scheduledTaskRunning=($q-match'(?i)Running|실행 중')}}catch{}
