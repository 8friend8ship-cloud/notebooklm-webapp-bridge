...
$HelperRaw='https://raw.githubusercontent.com/8friend8ship-cloud/notebooklm-webapp-bridge/main/local-agent/diagnostics/Test-NotebookLMFreshNotebookSourceCDPV3.ps1'
$OldNotebookId='69e055e5-c8d0-4e9c-8686-58cc6da35a51'
$Marker='NLM_FRESH_ALL_20260829_1915'
$DispatchMarker=Join-Path $Root 'AGENT_1.1.83_FRESH_NOTEBOOK_V3.dispatched'
$ResultMarker=Join-Path $Root 'AGENT_1.1.83_FRESH_NOTEBOOK_V3_RESULT.json'
$Stdout=Join-Path $Root 'AGENT_1.1.83_FRESH_NOTEBOOK_V3.stdout.log'
$Stderr=Join-Path $Root 'AGENT_1.1.83_FRESH_NOTEBOOK_V3.stderr.log'
...
function EnsureHelper{
  ...
  $tmp=$Helper+'.raw.download'
  Invoke-WebRequest -UseBasicParsing -Uri ($HelperRaw+'?hdcb='+[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()) -OutFile $tmp -TimeoutSec 30
  $actual=(GitBlobSha1 $tmp).ToLowerInvariant();if($actual -ne $ExpectedHelperSha){Remove-Item $tmp -Force -ErrorAction SilentlyContinue;throw "HELPER_RAW_SHA_MISMATCH actual=$actual expected=$ExpectedHelperSha"}
...
$mutex=New-Object Threading.Mutex($false,'HomeDesignFreshNotebook1183V3');$locked=$false;try{$locked=$mutex.WaitOne(0,$false)}catch{$locked=$false}
if(-not $locked){$skip=[ordered]@{ok=$false;action='AGENT_1.1.83_FRESH_NOTEBOOK_V3';state='SKIP_CONCURRENT';version=$Version;at=(Get-Date).ToString('o');normalChromeTouched=$false;bridgeChanged=$false;oauthChanged=$false;scopeChanged=$false};$skip|ConvertTo-Json -Compress;exit 0}
$prior=ReadJson $ResultMarker
if($prior){[void](SaveCentral 'AGENT_1.1.83_FRESH_NOTEBOOK_V3_RESULT.json' $prior);$prior|ConvertTo-Json -Depth 40 -Compress;if([bool]$prior.ok){exit 0}else{exit 2}}
if(Test-Path -LiteralPath $DispatchMarker){$pending=[ordered]@{ok=$false;action='AGENT_1.1.83_FRESH_NOTEBOOK_V3';state='DISPATCH_MARKER_PRESENT_NO_RESULT';version=$Version;at=(Get-Date).ToString('o');normalChromeTouched=$false;bridgeChanged=$false;oauthChanged=$false;scopeChanged=$false};[void](SaveCentral 'AGENT_1.1.83_FRESH_NOTEBOOK_V3_PENDING.json' $pending);$pending|ConvertTo-Json -Compress;try{$mutex.ReleaseMutex()}catch{};exit 2}

$result=[ordered]@{ok=$false;action='AGENT_1.1.83_FRESH_NOTEBOOK_V3';state='STARTING';version=$Version;taskId='LOCAL_NLM_FRESH_NOTEBOOK_SOURCE_CDP_V2_20260829_2132_01';helperSha=$ExpectedHelperSha;helperFetch='';startedAt=(Get-Date).ToString('o');freshNotebook=$false;sourceAdded=$false;sourceVerified=$false;notebookUrl='';notebookId='';previousNotebookId=$OldNotebookId;marker=$Marker;adoptedExistingFreshTab=$false;helperExit=$null;helperTimedOut=$false;helperStage='';normalChromeTouched=$false;bridgeChanged=$false;oauthChanged=$false;scopeChanged=$false;error='';centralPath=''}
try{
  $result.helperFetch=EnsureHelper
  $dispatch=[ordered]@{ok=$true;action='AGENT_1.1.83_FRESH_NOTEBOOK_V3_DISPATCH';state='DISPATCHED_ONCE';version=$Version;at=(Get-Date).ToString('o');taskId=$result.taskId;helperSha=$ExpectedHelperSha;helperFetch=$result.helperFetch;bridgeGenerationLane='0.2.39_UNCHANGED';normalChromeTouched=$false;oauthChanged=$false;scopeChanged=$false}
  $dispatch|ConvertTo-Json -Depth 20|Set-Content -LiteralPath $DispatchMarker -Encoding UTF8;[void](SaveCentral 'AGENT_1.1.83_FRESH_NOTEBOOK_V3_DISPATCH.json' $dispatch)
  ...
  $done=$p.WaitForExit(180000);if(-not $done){$result.helperTimedOut=$true;KillTree ([int]$p.Id);throw 'FRESH_V3_PROCESS_TIMEOUT_180S'}
  ...
  $result.ok=([int]$result.helperExit -eq 0 -and [bool]$payload.ok -and $result.freshNotebook -and $result.sourceAdded -and $result.sourceVerified -and $validId -and $validUrl);$result.state=if($result.ok){'PASS'}else{'FAIL'};if(-not $result.ok -and -not $result.error){$result.error='FRESH_NOTEBOOK_V3_GATE_FAILED'}
}catch{$result.state='ERROR';$result.error=$_.Exception.Message}
finally{$result.completedAt=(Get-Date).ToString('o');$result|ConvertTo-Json -Depth 40|Set-Content -LiteralPath $ResultMarker -Encoding UTF8;$result.centralPath=SaveCentral 'AGENT_1.1.83_FRESH_NOTEBOOK_V3_RESULT.json' $result;$result|ConvertTo-Json -Depth 40|Set-Content -LiteralPath $ResultMarker -Encoding UTF8;if($locked){try{$mutex.ReleaseMutex()}catch{}};try{$mutex.Dispose()}catch{}}
