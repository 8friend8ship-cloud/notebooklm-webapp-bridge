...
$Patched=Join-Path $Root 'HomeDesignLocalAgent-1.1.21-patched.ps1'
...
function StopStalePredecessorAgents([int]$MaxAgeSeconds=600){
  $Killed=@();$Now=Get-Date
  try{
    foreach($P in @(Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" -ErrorAction SilentlyContinue)){
      ...
      try{& taskkill.exe /PID ([int]$P.ProcessId) /T /F 2>$null|Out-Null;$Killed+=[ordered]@{pid=[int]$P.ProcessId;ageSeconds=[int]$Age}}catch{}
    }
  }catch{}
  return @($Killed)
}
...
$NewVersion="`$AgentVersion='1.1.21'"
...
$NewMutex="'HomeDesignLocalAgent121'"
...
$Killed=StopStalePredecessorAgents 600
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $Patched
$Rc=$LASTEXITCODE
if(@($Killed).Count -gt 0){[ordered]@{action='AGENT_1.1.21_STALE_PREDECESSOR_RECOVERY';killed=$Killed;exitCode=$Rc;at=(Get-Date).ToString('o')}|ConvertTo-Json -Depth 10 -Compress|Write-Output}
exit $Rc
