...
function Process-One {
  ...
  $canary=[bool]$t.canary;$generationAllowed=[bool]$t.generationAllowed
  if(-not $canary -and -not $generationAllowed){return @{ok=$true;status='HOLD_GENERATION_NOT_ALLOWED';executed=$false;taskId=$taskId}}
  if($generationAllowed -and [string]$t.generationGateEvidence -ne 'BIND08_VERIFIED_X2'){return @{ok=$false;status='HOLD_GENERATION_GATE_EVIDENCE_MISSING';executed=$false;taskId=$taskId}}
  ...
  try{
    ...
    $r=@{ok=$ok;status=$(if($ok){'EXECUTOR_IMAGE_READY'}else{'EXECUTOR_ADAPTER_FAILED'});executed=$true;taskId=$taskId;canary=$canary;generationAllowed=$generationAllowed;adapterExitCode=$rc;adapterReceipt=$receipt;genericDownloadsScan=$false;duplicateGenerationAllowed=$false}
    $rp=Save-Result $r $taskId
    Move-Item -LiteralPath $claimed -Destination (Join-Path $(if($ok){$done}else{$failed}) $taskFile.Name) -Force
    $r.resultPath=$rp;return $r
  }catch{
    $r=@{ok=$false;status='EXECUTOR_EXCEPTION';executed=$true;taskId=$taskId;error=$_.Exception.Message;genericDownloadsScan=$false;duplicateGenerationAllowed=$false};$rp=Save-Result $r $taskId
