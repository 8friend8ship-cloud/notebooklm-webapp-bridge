...
if (-not $download) {
  ...
  throw ('VIDEO_EXISTING_DOWNLOAD_FAILED:' + ($summary | ConvertTo-Json -Compress))
}
...
if (-not (Test-Path -LiteralPath $sourcePath -PathType Leaf)) { throw ('VIDEO_SOURCE_PATH_MISSING:' + $sourcePath) }

$rawQueens = & $queensScript -TaskId $TaskId -ArtifactType 'VIDEO_OVERVIEW' -SourcePath $sourcePath -TimeoutSeconds $TimeoutSeconds 2>&1
$qExit = $LASTEXITCODE
$qLines = @($rawQueens | ForEach-Object { [string]$_ })
$qJsonLine = @($qLines | Where-Object { $_ -match '^\s*\{' } | Select-Object -Last 1)
if ($qExit -ne 0 -or $qJsonLine.Count -lt 1) { throw ('VIDEO_QUEENS_FIRST_FAILED:exit=' + $qExit + ';output=' + ($qLines -join ' | ')) }
...
[pscustomobject]@{
  ok = $true
  action = 'NOTEBOOKLM_EXISTING_VIDEO_QUEENS_FIRST_V1'
  taskId = $TaskId
  artifactType = 'VIDEO_OVERVIEW'
