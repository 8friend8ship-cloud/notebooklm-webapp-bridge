param(
  ...
  [string]$DriveFileId='',
  [bool]$QueensRegistered=$false,
  [int]$ResultAckCount=0,
  [bool]$ChangedEvidence=$true
)
...
}elseif(($fallbackReasons -contains $FlowFailureReason) -and $ChangedEvidence){
...
$functionalPass=($fileOk -and $driveOk -and $QueensRegistered -and $ResultAckCount -ge 2)
...
[ordered]@{
  ...
  queensRegistered=$QueensRegistered
  resultAckCount=$ResultAckCount
  requiredResultAckCount=2
  functionalPass=$functionalPass
  completion=$completion
  nextGate=$(if(-not $fileOk){'REAL_IMAGE_DOWNLOAD'}elseif(-not $driveOk){'DRIVE_READBACK'}elseif(-not $QueensRegistered){'QUEENS_REGISTRATION'}elseif($ResultAckCount -lt 2){'RESULT_ACK_X2'}else{'DONE'})
