...
if($CreateLocalSaveSmoke){
  try{
    $dir=Join-Path $Base 'CaptureBridge\INBOX\NotebookLM'
...
if($resolvedKind -eq 'AUTO'){if($ExpectedMediaSeconds -gt 0){$resolvedKind='VIDEO'}else{$resolvedKind='TEXT'}}
...
$out=[ordered]@{ok=$ok;action='NOTEBOOKLM_CLAIM_START_BRIDGE_TEST_V2';taskId=$TaskId;state=$state;checkedAt=(Get-Date).ToString('o');taskKind=$resolvedKind;expectedMediaSeconds=$ExpectedMediaSeconds;startGraceSeconds=$StartGraceSeconds;claimedAgeSeconds=$ClaimedAgeSeconds;workBudgetSeconds=$workBudget;runAgeSeconds=$runAgeSeconds;hostHealthy=$(if($health){[bool]$health.ok}else{$false});hostVersion=$(if($health){[string]$health.version}else{''});hostAsyncJobs=$(if($health){[bool]$health.asyncJobs}else{$false});hostResult=$hostResult;hostHealthError=$healthError;hostResultError=$hostResultError;localStatusExists=(Test-Path -LiteralPath $StatusPath);localResultExists=(Test-Path -LiteralPath $ResultPath);wrapperPid=$wrapperPid;wrapperAlive=$wrapperAlive;statusPath=$StatusPath;resultPath=$ResultPath;outputProgress=$outputProgress;problem=$problem;immediateAction=$(switch($state){'DONE' {'NO_ACTION'};'PROGRESSING' {'NO_ACTION_OUTPUT_IS_CHANGING'};'START_CONFIRMED' {'NO_ACTION_WITHIN_MEDIA_BUDGET'};'CLAIMED_PENDING_START' {'WAIT_SHORT_START_GRACE_ONLY'};'CLAIMED_START_DELAY' {'CHECK_POLLER_AND_HOST_HANDOFF_NOW'};'RUNNING_OVER_BUDGET' {'CHECK_OUTPUT_PROGRESS_THEN_DIAGNOSE_WORKER'};'START_LOST' {'DIAGNOSTIC_HOLD_THEN_MINIMUM_RECOVERY'};'HOST_DOWN' {'RECOVER_EXISTING_HOST_ONLY'};'HOST_TASK_ERROR' {'READ_HOST_ERROR_THEN_MINIMUM_FIX'};default {'DIAGNOSE'}})}

if($WriteCentralReadback){$central=Find-CentralRoot;if($central){try{$dir=Join-Path $central 'Runtime_Readback';New-Item -ItemType Directory -Force -Path $dir|Out-Null;$dest=Join-Path $dir ('NOTEBOOKLM_CLAIM_START_'+$TaskId+'.json');$out|ConvertTo-Json -Depth 30|Set-Content -LiteralPath $dest -Encoding UTF8;$out.centralReadback=$dest}catch{$out.centralReadbackError=$_.Exception.Message}}else{$out.centralReadbackError='CENTRAL_DRIVE_ROOT_NOT_FOUND'}}
