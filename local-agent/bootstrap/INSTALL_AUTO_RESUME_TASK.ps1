...
if(Test-Path -LiteralPath $Watchdog -PathType Leaf){
  try{$directExit=RunDirectWatchdog $DirectWatchdogTimeoutSeconds;$immediateMode='DIRECT_WATCHDOG_ALWAYS';if($directExit-ne0){$errors+=('DIRECT_WATCHDOG_EXIT_'+$directExit)}}catch{$errors+=('DIRECT_WATCHDOG:'+ $_.Exception.Message);$directExit=3}
