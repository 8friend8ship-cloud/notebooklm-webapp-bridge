...
function RunRdcDataPlaneGuard{
  ...
  try{
    ...
    $r.guardExit=$LASTEXITCODE
    $r.guardOutput=$raw.Trim()
    $r.ok=([int]$LASTEXITCODE-eq0)
  }catch{$r.ok=$false;$r.errors+=$_.Exception.Message}
  $r.completedAt=(Get-Date).ToString('o')
  SaveRepair $r
}
RunRdcDataPlaneGuard
