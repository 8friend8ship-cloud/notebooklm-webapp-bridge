...
if(-not(Test-Path -LiteralPath $verifyMarker)){
  ...
  try{
    ...
    foreach($drv in @(Get-PSDrive -PSProvider FileSystem -ErrorAction SilentlyContinue)){
      $rr=[string]$drv.Root;if(-not $rr){continue}
      foreach($cand in @((Join-Path $rr $centralName),(Join-Path $rr ($myDriveKo+'\'+$centralName)),(Join-Path $rr ('My Drive\'+$centralName)),(Join-Path $rr ('Google Drive\'+$centralName)))){
        if(Test-Path -LiteralPath $cand){$dest=Join-Path $cand 'Runtime_Readback';New-Item -ItemType Directory -Force -Path $dest|Out-Null;$json|Set-Content -LiteralPath (Join-Path $dest ('NOTEBOOKLM_CDP_DOWNLOAD_'+$verifyKey+'.json')) -Encoding UTF8;break}
      }
    }
  }catch{}
...
while((Get-Date)-lt $deadline){
  ...
  if(Test-Path -LiteralPath $StateFile){
    try{
      ...
      if($av -eq $targetAgent -and $hr -and $bv -eq $targetBridge -and $gov -and $sync){Write-Host 'RESUME RESULT: ACTIVE + GOVERNOR VERIFIED';exit 0}
      if($av -eq $targetAgent -and $hr -and $bv -eq $targetBridge -and $directExit -eq 0){Write-Host 'RESUME RESULT: ACTIVE; GOVERNOR READBACK STILL SYNCING';exit 0}
    }catch{}
  }
}
