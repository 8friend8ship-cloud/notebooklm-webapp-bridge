...
$Version='OPENAI_WEB_SYNC_GUARD_V1_20260910'
...
function Get-ChatWindows{
 ...
 foreach($e in @($tops)){try{$c=$e.Current;$pid=[int]$c.ProcessId;$hwnd=[int64]$c.NativeWindowHandle;$title=[string]$c.Name;if($pid-le0-or$hwnd-le0-or[string]::IsNullOrWhiteSpace($title)){continue};$p=Get-Process -Id $pid -ErrorAction Stop;$name=[string]$p.ProcessName;if($name-notmatch'(?i)^(chrome|msedge|chatgpt)$'){continue};if($title-match'(?i)(ChatGPT|OpenAI)'){$rows+=[pscustomobject]@{pid=$pid;hwnd=$hwnd;process=$name;title=$title;foreground=([OaiSyncWin]::GetForegroundWindow().ToInt64()-eq$hwnd)}}}catch{}}
 return @($rows)
}
...
$refreshAttempted=$false;$refreshOk=$false;$target=$null;$error=''
if($eligible){
 try{
