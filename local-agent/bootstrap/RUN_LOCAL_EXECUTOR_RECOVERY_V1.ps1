...
function ReadFlowLane([string]$Path){
  try{if(-not(Test-Path -LiteralPath $Path -PathType Leaf)){return $null};$j=Get-Content -LiteralPath $Path -Raw -Encoding UTF8|ConvertFrom-Json;return [pscustomobject]@{appliedVersion=[string]$j.appliedVersion;exitCode=$(if($j.exitCode-ne$null){[int]$j.exitCode}else{$null});updatedAt=[string]$j.updatedAt}}catch{return $null}
}
function WaitFlowLane([string]$Path,[int]$Seconds=330){
  $deadline=(Get-Date).AddSeconds($Seconds)
  do{$lane=ReadFlowLane $Path;if($lane-and$lane.appliedVersion-eq'1.1.69'){return $lane};Start-Sleep -Seconds 5}while((Get-Date)-lt$deadline)
  return (ReadFlowLane $Path)
}
...
$flowLaneApplied=$false;$flowLaneAppliedVersion='';$flowLaneExitCode=$null;$flowLaneUpdatedAt='';$flowLaneWaitSeconds=330
...
$FlowLaneState=Join-Path $Root 'state-flow.json'
...
}else{$errors+='AUTORESUME_GATE_BLOCKED_FLOW_RECOVERY'}

# The independent Flow lane is the canonical answer to global stable starvation.
# Require evidence that current Bootstrap has applied/attempted pinned Flow 1.1.69
# before the direct R10 fallback is allowed.
if($autoResumeInstallExit-eq0 -and $periodicTriggerReady -and $persistenceReady -and $autoResumeImmediateVerified -and $bootstrapLoopPresent){
  $lane=WaitFlowLane $FlowLaneState $flowLaneWaitSeconds
  if($lane){$flowLaneAppliedVersion=[string]$lane.appliedVersion;$flowLaneExitCode=$lane.exitCode;$flowLaneUpdatedAt=[string]$lane.updatedAt;$flowLaneApplied=($flowLaneAppliedVersion-eq'1.1.69')}
  if(-not$flowLaneApplied){$errors+='FLOW_INDEPENDENT_LANE_1.1.69_NOT_APPLIED'}
}
...
if($autoResumeInstallExit-eq0 -and $periodicTriggerReady -and $persistenceReady -and $autoResumeImmediateVerified -and $bootstrapLoopPresent -and (Test-Path -LiteralPath $AlwaysOnUpdater -PathType Leaf)){
  try{& powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $AlwaysOnUpdater;$alwaysOnSyncExit=$LASTEXITCODE;if($alwaysOnSyncExit-ne0){$errors+=('ALWAYS_ON_AUX_SYNC_EXIT_'+$alwaysOnSyncExit)}}catch{$errors+=('ALWAYS_ON_AUX_SYNC_RUN:'+ $_.Exception.Message)}
  $alwaysOnReceiptFresh=FreshReceipt $AlwaysOnReceipt $startedAt
  if($alwaysOnReceiptFresh){
    try{$ao=Get-Content -LiteralPath $AlwaysOnReceipt -Raw -Encoding UTF8|ConvertFrom-Json;$alwaysOnSyncOk=[bool]$ao.ok;$alwaysOnReloadVerified=[bool]$ao.cdpReloadVerified;$alwaysOnReloadPending=[bool]$ao.reloadPending;$alwaysOnRuntimeVersion=[string]$ao.runtimeVersion;$alwaysOnUpdatedTargetCount=@($ao.updatedTargets).Count;if($alwaysOnSyncOk-and$alwaysOnReloadVerified-and$alwaysOnRuntimeVersion-eq'1.0.2'){$alwaysOnRuntimeState='V1.0.2_RUNTIME_VERIFIED'}elseif($alwaysOnSyncOk){$alwaysOnRuntimeState='V1.0.2_FILES_SYNCED_RELOAD_PENDING'}else{$alwaysOnRuntimeState='AUX_SYNC_RECEIPT_FAILED'}}catch{$errors+=('ALWAYS_ON_AUX_RECEIPT_READ:'+ $_.Exception.Message);$alwaysOnRuntimeState='AUX_SYNC_RECEIPT_READ_FAILED'}
...
if($autoResumeInstallExit-eq0 -and $periodicTriggerReady -and $persistenceReady -and $autoResumeImmediateVerified -and $bootstrapLoopPresent -and $flowLaneApplied){
  try{
    if(Test-Path -LiteralPath $FlowVerifier){
      $central=FindCentral;$args=@('-NoProfile','-NonInteractive','-ExecutionPolicy','Bypass','-File',$FlowVerifier);if($central){$args+=@('-CentralRootOverride',$central)}
      $out=& powershell.exe @args 2>&1;$flowCanonicalExit=$LASTEXITCODE;$last=($out|Select-Object -Last 1|Out-String).Trim();if($last){try{$j=$last|ConvertFrom-Json;$flowCanonicalOk=[bool]$j.ok;$flowCanonicalSelected=[string]$j.selectedPath}catch{}};if($flowCanonicalExit-ne0-or-not$flowCanonicalOk){$errors+=('FLOW_CANONICAL_EXIT_'+$flowCanonicalExit)}
    }
...
if(-not$flowReceiptFresh -and $autoResumeInstallExit-eq0 -and $periodicTriggerReady -and $persistenceReady -and $autoResumeImmediateVerified -and $bootstrapLoopPresent -and $flowLaneApplied -and $flowCanonicalOk -and (Test-Path -LiteralPath $FlowAgent -PathType Leaf)){
