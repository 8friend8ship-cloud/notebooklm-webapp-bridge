...
$Version='REMOTE_DC_DATA_PLANE_GUARD_V2_0250_20260911'
...
$o=[ordered]@{ok=$true;action='REMOTE_DC_DATA_PLANE_ONE_SHOT_RECOVERY';version=$Version;package=$Package;requestId=$requestId;enabled=$enabled;alreadyCompleted=[bool]$already;prewarmOk=$false;prewarmExit=$null;remoteBefore=[int]$before.Count;tcpBefore=[int]$tcpBefore;stopped=@();started=$false;remoteAfter=0;tcpAfter=0;cloudDataPlaneVerified=$false;cloudVerificationRequired='GET_CONFIG_OR_LIST_PROCESSES_PLUS_FILE_RW_X2';broadNodeKill=$false;globalNpmCacheTouched=$false;globalExecutionPolicyChanged=$false;startedAt=(Get-Date).ToString('o');completedAt='';error=''}
if($enabled-and$requestId-and-not$already){try{$warm=WarmCache;$o.prewarmOk=[bool]$warm.ok;$o.prewarmExit=[int]$warm.exitCode;if(-not$warm.ok){throw ('PREWARM_FAILED:'+([string]$warm.output))};$o.stopped=@(StopExact $before);Start-Sleep -Seconds 2;$launch=StartRemote;$o.started=[bool]$launch.ok;if(-not$launch.ok){throw [string]$launch.error};$after=GetRemote (GetAll);$o.remoteAfter=[int]$after.Count;$o.tcpAfter=[int](TcpCount $after);$o.ok=($o.remoteAfter-gt0-and$o.tcpAfter-gt0);if($o.ok){[ordered]@{completedRequestId=$requestId;completedAt=(Get-Date).ToString('o');version=$Version;package=$Package}|ConvertTo-Json|Set-Content -LiteralPath $StatePath -Encoding UTF8}}catch{$o.ok=$false;$o.error=$_.Exception.Message}}
