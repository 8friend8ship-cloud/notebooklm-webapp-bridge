...
$Version='WATCHDOG_V14_CENTRAL_TAB_MIDCLEAN_RECOVERY_20260909'
...
if($hold){
  ...
  Save $Receipt 'WATCHDOG_LAST.json' ([ordered]@{ok=$ok;action='WATCHDOG_TABLET_PRIMARY_HOLD_V11_FLOW_AUTORESTORE';version=$Version;startedAt=$start;completedAt=(Get-Date).ToString('o');hostHealthy=(HostHealthy);hostRestoreBefore=[bool]$hostRestore.before;hostRestoreRestarted=[bool]$hostRestore.restarted;hostRestoreOk=[bool]$hostRestore.ok;hostRestoreError=[string]$hostRestore.error;flowCdp9224Before=[bool]$flowRestore.before;flowCdp9224Started=[bool]$flowRestore.started;flowCdp9224Ok=[bool]$flowRestore.ok;flowCdp9224Error=[string]$flowRestore.error;tabletPrimaryOwner=$true;notebookLaptopHeld=$true;flowLaptopFallbackEnabled=$true;stableMetaReachable=$true;stableMetaTransport=[string]$s.transport;stableEnabled=$false;stableNotes=[string]$s.notes;bootstrapBefore=[bool]$bootstrap.beforePresent;bootstrapRefreshed=[bool]$bootstrap.refreshed;bootstrapRestarted=[bool]$bootstrap.restarted;bootstrapOk=[bool]$bootstrap.ok;bootstrapSha=[string]$bootstrap.sha;bootstrapTransport=[string]$bootstrap.transport;bootstrapError=[string]$bootstrap.error;currentVersion=(CurrentVersion);notebooklmRuntimeChecked=$true;dedicatedNotebookChromeBefore=[int]$shutdown.before;dedicatedNotebookChromeStopped=[int]$shutdown.stopped;dedicatedNotebookChromeAfter=[int]$shutdown.after;autoResumeInvoked=$false;independentLanesExpected='FLOW,IMAGE,APPSCRIPT';normalChromeTouched=$false;oauthChanged=$false;scopeChanged=$false;exitCode=($(if($ok){0}else{4}))})
