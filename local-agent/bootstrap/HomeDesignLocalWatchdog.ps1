...
SaveEntryReceipt ([ordered]@{ok=$true;action='WATCHDOG_ENTRY_V4';version='WATCHDOG_SINGLE_EXECUTOR_TIMEOUT_ALIGNED_20260829';pid=$PID;startedAt=$started;autoResumeTimeoutSeconds=$AutoResumeTimeoutSeconds;normalChromeTouched=$false;oauthChanged=$false;scopeChanged=$false})
...
if($hostOk -and $bootstrapOk -and $stateOk -and $versionOk){
  ...
  SaveReceipt ([ordered]@{ok=$true;action='WATCHDOG_PASS_V4';version='WATCHDOG_SINGLE_EXECUTOR_TIMEOUT_ALIGNED_20260829';startedAt=$started;completedAt=(Get-Date).ToString('o');hostHealthy=$true;bootstrapLoopPresent=$true;stateFresh=$true;currentVersion=$currentVersion;targetVersion=$targetVersion;autoResumeInvoked=$false;timedOut=$false;timeoutSeconds=$AutoResumeTimeoutSeconds;exitCode=0})
  exit 0
}
...
if(-not(Test-Path -LiteralPath $AutoResume)){
  ...
  SaveReceipt ([ordered]@{ok=$false;action='AUTO_RESUME_MISSING';version='WATCHDOG_SINGLE_EXECUTOR_TIMEOUT_ALIGNED_20260829';startedAt=$started;completedAt=(Get-Date).ToString('o');currentVersion=$currentVersion;targetVersion=$targetVersion;autoResumeInvoked=$false;timedOut=$false;timeoutSeconds=$AutoResumeTimeoutSeconds;exitCode=2})
  exit 2
}
...
try{
  ...
  SaveReceipt ([ordered]@{ok=[bool]($rc -eq 0);action='AUTO_RESUME_COMPLETED_V4';version='WATCHDOG_SINGLE_EXECUTOR_TIMEOUT_ALIGNED_20260829';startedAt=$started;completedAt=(Get-Date).ToString('o');currentVersion=$currentVersion;targetVersion=$targetVersion;autoResumeInvoked=$true;timedOut=$false;timeoutSeconds=$AutoResumeTimeoutSeconds;exitCode=$rc;hostAfter=$hostAfter;bootstrapAfter=$bootstrapAfter})
  exit $rc
}catch{Log ('WATCHDOG_EXCEPTION '+$_.Exception.Message);SaveReceipt ([ordered]@{ok=$false;action='WATCHDOG_EXCEPTION';version='WATCHDOG_SINGLE_EXECUTOR_TIMEOUT_ALIGNED_20260829';startedAt=$started;completedAt=(Get-Date).ToString('o');currentVersion=$currentVersion;targetVersion=$targetVersion;autoResumeInvoked=$true;timedOut=$false;timeoutSeconds=$AutoResumeTimeoutSeconds;exitCode=3;error=$_.Exception.Message});exit 3}
