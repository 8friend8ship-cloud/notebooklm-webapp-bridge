...
$Version='CENTRAL_AGENT_TAB_SUPERVISOR_V5_AUTH_TTL_POWERGUARD_PS51_20260909'
...
function Test-Host {
  try{$h=Invoke-RestMethod 'http://127.0.0.1:8765/health' -TimeoutSec 3;return [bool]$h.ok}catch{return $false}
}
function Test-Cdp {
  try{$v=Invoke-RestMethod 'http://127.0.0.1:9224/json/version' -TimeoutSec 3;return [bool]$v.Browser}catch{return $false}
}
function Test-Remote {
  try{
    $p=@(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue|Where-Object{[string]$_.CommandLine -match '(?i)desktop-commander' -and [string]$_.CommandLine -match '(?i)(?:^|\s)remote(?:\s|$)'})
    if($p.Count -eq 0){return $false}
    $ids=@($p.ProcessId)
    return (@(Get-NetTCPConnection -State Established -ErrorAction SilentlyContinue|Where-Object{$ids -contains $_.OwningProcess}).Count -gt 0)
  }catch{return $false}
}
...
function Refresh-Verified([string]$RepoPath,[string]$Dest){
  try{
    $u='https://api.github.com/repos/'+$Repo+'/contents/'+$RepoPath+'?ref=main&cb='+[DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
    $x=Invoke-RestMethod -Uri $u -Headers @{'User-Agent'='HomeDesign-Central-Supervisor';'Accept'='application/vnd.github+json'} -TimeoutSec 15
    $b=[Convert]::FromBase64String(([string]$x.content -replace '\s',''));$sha=(GitBlob $b).ToLowerInvariant()
    if($sha -ne ([string]$x.sha).ToLowerInvariant()){throw 'SHA_MISMATCH'}
    $tmp=$Dest+'.download';[IO.File]::WriteAllBytes($tmp,$b);Move-Item $tmp $Dest -Force
    return $sha
  }catch{return ''}
}
function Get-PowerGuardProcesses {
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue|Where-Object{[string]$_.CommandLine -match '(?i)CentralAgentPowerContinuityGuard\.ps1'}
}
function Read-PowerGuardReceipt {
  try{if(Test-Path $PowerGuardReceipt){return (Get-Content $PowerGuardReceipt -Raw -Encoding UTF8|ConvertFrom-Json)}}catch{}
  return $null
}
function Ensure-PowerGuard {
  ...
  try{
    ...
    $procs=@(Get-PowerGuardProcesses);$o.before=$procs.Count
    $oldReceipt=Read-PowerGuardReceipt;$oldVersion=$(if($oldReceipt){[string]$oldReceipt.version}else{''})
    if($procs.Count -gt 0 -and $oldVersion -notmatch '^POWER_CONTINUITY_GUARD_V3_'){
      foreach($p in $procs){try{Stop-Process -Id ([int]$p.ProcessId) -Force -ErrorAction Stop}catch{}}
      Start-Sleep -Seconds 1;$o.restartedForVersion=$true
    }
    if(@(Get-PowerGuardProcesses).Count -eq 0){
      ...
      $o.started=$true;Start-Sleep -Seconds 3
    }
    $o.after=@(Get-PowerGuardProcesses).Count
    $rr=Read-PowerGuardReceipt
    if($rr){
      $o.receiptVersion=[string]$rr.version;$o.systemRequiredHeld=[bool]$rr.systemRequiredHeld;$o.acPower=[bool]$rr.acPower;$o.remoteProcessPresent=[bool]$rr.remoteProcessPresent
    }
    $needsHold=[bool]($o.acPower -and $o.remoteProcessPresent)
    $o.ok=[bool]($o.after -gt 0 -and $o.receiptVersion -match '^POWER_CONTINUITY_GUARD_V3_' -and ((-not $needsHold) -or $o.systemRequiredHeld))
...
function Run-JsonScript([string]$Path){
  ...
  try{
    ...
    try{$o.obj=$o.raw|ConvertFrom-Json}catch{
      ...
      if($jsonLines.Count -gt 0){$o.obj=$jsonLines[-1]|ConvertFrom-Json}else{throw}
    }
    $o.ok=[bool]$o.obj.ok
  }catch{$o.error=$_.Exception.Message}
  return [pscustomobject]$o
...
$powerGuardRun=Ensure-PowerGuard
if(-not $powerGuardRun.ok){$issues.Add('POWER_CONTINUITY_GUARD_DOWN')}

$activitySha=Refresh-Verified 'local-agent/bootstrap/WindowActivitySupervisor.ps1' $WindowActivityScript
$activityRun=$null
if(-not $activitySha){$issues.Add('WINDOW_ACTIVITY_REFRESH_FAILED')}
elseif(Test-Path $WindowActivityScript){
  $activityRun=Run-JsonScript $WindowActivityScript
  if(-not $activityRun.obj){$issues.Add('WINDOW_ACTIVITY_NO_READBACK')}
  elseif([string]$activityRun.obj.version -notmatch '^WINDOW_ACTIVITY_SUPERVISOR_V2_'){$issues.Add('WINDOW_ACTIVITY_VERSION_MISMATCH')}
  elseif(-not [bool]$activityRun.obj.ok){$issues.Add('WINDOW_ACTIVITY_NOT_OK')}
}else{$issues.Add('WINDOW_ACTIVITY_MISSING')}

$cleanupRun=$null
if(Test-Path $CleanupScript){
  $cleanupRun=Run-JsonScript $CleanupScript
  if(-not $cleanupRun.obj){$issues.Add('CLEANUP_NO_READBACK')}
  elseif(-not [bool]$cleanupRun.obj.ok){$issues.Add('CLEANUP_NOT_OK')}
...
$registryCount=0;$registryOk=$true;$r=@()
...
if($registryOk -and $cleanupRun -and $cleanupRun.obj -and $cleanupRun.obj.results){
  foreach($rr in @($cleanupRun.obj.results)){
    if([string]$rr.state -match '^(ALREADY_NOT_VISIBLE|CLOSED_EXACT_HWND|HWND_REUSED_OWNER_MISMATCH|ALREADY_CLOSED|CLOSED)$' -and [string]$rr.runId){$pruneIds+=[string]$rr.runId}
  }
  $pruneIds=@($pruneIds|Sort-Object -Unique)
}
if($registryOk -and $pruneIds.Count -gt 0){
  try{
    $kept=@($r|Where-Object{$pruneIds -notcontains [string]$_.runId})
    if($kept.Count -lt $registryCount){
      $tmp=$Registry+'.compact';ConvertTo-Json -InputObject ([object[]]$kept) -Depth 30|Set-Content -LiteralPath $tmp -Encoding UTF8;Move-Item $tmp $Registry -Force
      $registryPruned=$registryCount-$kept.Count;$registryCountAfter=$kept.Count
    }
  }catch{$issues.Add('REGISTRY_COMPACT_FAIL')}
}
...
$authDupRemain=$false
if($activityRun -and $activityRun.obj){
  if([int]$activityRun.obj.duplicateFamiliesRemaining -gt 0){$authDupRemain=$true}
  if([int]$activityRun.obj.ttlCloseFailures -gt 0){$issues.Add('AUTH_TTL_CLOSE_FAILURE')}
}
if(-not $authDupRemain -and $cleanupRun -and $cleanupRun.obj -and $cleanupRun.obj.authActions){
  foreach($a in @($cleanupRun.obj.authActions)){
    if([int]$a.found -gt 1 -and [int]$a.closed -eq 0 -and [string]$a.action -notmatch 'PROTECT_WAITING_USER'){$authDupRemain=$true}
  }
}
...
if($issues.Count -gt 0 -and (Test-Path $RecoveryScript)){
  $recent=$false
  try{if(Test-Path $RecoveryReceipt){$recent=(((Get-Date)-(Get-Item $RecoveryReceipt).LastWriteTime).TotalSeconds -lt 120)}}catch{}
  if($recent -and -not $SelfTestRecovery){$recoveryCooldown=$true}else{
    try{$reason=($issues -join ',');& powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $RecoveryScript -Reason $reason|Out-Null;$recoveryExit=$LASTEXITCODE;$recoveryTriggered=$true}catch{$recoveryExit=9}
  }
}

$hostAfter=Test-Host;$cdpAfter=Test-Cdp;$remoteAfter=Test-Remote
$activityObj=$(if($activityRun){$activityRun.obj}else{$null})
$cleanupObj=$(if($cleanupRun){$cleanupRun.obj}else{$null})
$activityHealthy=[bool]($activityObj -and [bool]$activityObj.ok -and [string]$activityObj.version -match '^WINDOW_ACTIVITY_SUPERVISOR_V2_')
...
$out=[ordered]@{
  ...
  version=$Version;startedAt=$started;completedAt=(Get-Date).ToString('o');phase='POWER_V3+WINDOW_ACTIVITY_V2_AUTH_TTL+MID_CLEAN+VERIFY+RECOVER'
  ...
  cleanupExit=$(if($cleanupRun){$cleanupRun.exit}else{9});cleanupVersion=$(if($cleanupObj){[string]$cleanupObj.version}else{''});cleanupClosed=$(if($cleanupObj){[int]$cleanupObj.closedCount}else{-1})
